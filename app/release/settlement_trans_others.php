...
if(isset($_POST['key']))
{
	...
	$Credit_lube=mysqli_real_escape_string($con,$_POST['Credit_lube']);
   ...
   // $password=mysqli_real_escape_string($con,$_POST['']);
	...
	while($rows = mysqli_fetch_assoc($res))
     ...
     if($count == 1)
     {
		...
		$sql="insert into tbl_shift_settlement(Nozzle_Amount,Credit_lube,Cash,Petrocard,Debit_credit_card,
		Wallet,trans_by,Diff,shift_id,type) 
		values ('0','0','$Credit_lube','0','0','0',
		'0','$id','$Diff','$shift_id',2) ";
		...
		for ($i = 0; $i < count($modearr); $i++) 
			{
				...
				('$modearr[$i]','$last_id','$amount_arr[$i]',2)";
		 ...
		 $sql3="update shifts set lueb='1' where id='$shift_id'";
