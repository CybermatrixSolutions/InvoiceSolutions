   ...
   // $password=mysqli_real_escape_string($con,$_POST['']);
	...
	while($rows = mysqli_fetch_assoc($res))
     ...
     if($count == 1)
     {
		...
		and shift_id='$shift_id' and cust_type='walkin' and petrol_or_lube=2 
		and shift_id in (select id from shifts where lueb=0 and shift_manager='$id')
		...
		//where Reading_by='$id' and t4.is_active=1 and t3.is_active=1 and t2.is_active=1 and t6.id='$shift_id'
        ...
        $res2  = mysqli_query($con,$sql2);
		 ...
		 while($row = mysqli_fetch_assoc($res2))
		 {
          ...
          $json = array( "msg"=>"Settlement!" ,"key"=>$key,'Credit_lube'=>$trans_amount_l);
