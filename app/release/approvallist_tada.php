   ...
   // $password=mysqli_real_escape_string($con,$_POST['']);
	...
	while($rows = mysqli_fetch_assoc($res))
     ...
     if($count == 1)
     {
		...
		from tbl_employee_tada t1 inner join tbl_user_master t2 on t1.emp_code=t2.Emp_code where Approval='$Emp_code' and Approval_Status=0 order by id DESC";
        ...
        $res1  = mysqli_query($con,$sql);
		 ...
		 while($row = mysqli_fetch_assoc($res1))
		 {
          ...
          $json = array( "msg"=>"ta-da con approve!" ,"key"=>$key,'ta-da approve'=>$data);
